...
上述的程式可否通過 tsc 型態檢核嗎？ 答案很難回答，因為取決於“選項”的設定。
...
解決了一個疑問，心中是否又産生另一個疑問呢： 不是編譯失敗嗎？怎麼還會産生 js 檔案呢？原來 tsc 編譯器與其他傳統正統的程式語言編譯器不同，編譯過程産生錯誤與會不會産生 js 檔案不是正相關的，也就說即使編譯過程有錯誤産生也會産出 js 的結果來。那可否要求若有錯誤發生時不要産生結果檔案呢？可以的，透過“noEmitOnError”選項來控制，它的預設值是false，若你開啟 tsconfig.json “noEmitOnError”選項，並把它的值設成 true，則當編譯過程有錯誤時就不會還産生 js 檔案了。
...
|downlevelIteration| 程式中若使用到Symbol 完訪器，啟用此選項可更精確地對 ES5 環境模擬出適當的 ES2015 完訪器 |
...
|forceConsitentCasingInFileNames| 要求匯入的模組名稱必須和實際檔案名稱有一玫的大小寫 |
...
|lib| 指定編譯器要選使用的定義檔 |
|module| 指定用那種棤組規範來進行編譯  |
|moduleResoultion| 惷定模組的解析方式(可設為相容於 TypeScript 舊版的 "classic" 或新版的 "node")|
...
| amd | 指定使用 AMD (Asynchronous Module Definition, 非同步模組)，是 RequireJS模組載入工具支擾的規範 |
| system | 指定使用 SystemJS 模組載入工具支援的模組規範  |
| umd | 指定使用 UMD (Universal Module Definition, 通用模姐定義) 規範   |
| es2015/es6 | 指定使用 ES2015 的 ECMAScript 模組規範 |
| es2020 | 指定使用 ES2020 的 ECMAScript 模組規範 |
| esnext | 指定使用下一版 JavaScript 的模組規範  |


設定編譯時要加入的函式庫， compilerOption中 lib 屬性選項
|名稱|說明|
|-----------|-----------------------------------------------------------------------------|
| es5, es2015, es2016, 2017... |  |
| esnext |  |
| dom |  |
| dom.iterable |  |
| scriptHost |  |
